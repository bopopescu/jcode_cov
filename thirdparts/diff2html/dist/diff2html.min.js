...
function(modules) {
    ...
    __webpack_require__(0)
        ...
        function() {
            ...
            htmlPrinter = __webpack_require__(20).HtmlPrinter;
            ...
            Diff2Html.prototype.getPrettyHtml = function(diffInput, config) {
                ...
                var diffOutput = "";
                return diffOutput = "side-by-side" === configOrEmpty.outputFormat ? htmlPrinter.generateSideBySideJsonHtml(diffJson, configOrEmpty) : htmlPrinter.generateLineByLineJsonHtml(diffJson, configOrEmpty),
                fileList + diffOutput
            },
            Diff2Html.prototype.getPrettyHtmlFromDiff = function(diffInput, config) {
                var configOrEmpty = config || {};
                return configOrEmpty.inputFormat = "diff",
                configOrEmpty.outputFormat = "line-by-line",
                this.getPrettyHtml(diffInput, configOrEmpty)
            },
            Diff2Html.prototype.getPrettyHtmlFromJson = function(diffJson, config) {
                var configOrEmpty = config || {};
                return configOrEmpty.inputFormat = "json",
                configOrEmpty.outputFormat = "line-by-line",
                this.getPrettyHtml(diffJson, configOrEmpty)
            },
...
function(module, exports, __webpack_require__) { !
    function() {
        ...
        LINE_TYPE = {
            INSERTS: "d2h-ins",
            INSERTS_MISS: "d2h-ins-miss",
            INSERTS_PART: "d2h-ins-part",
            INSERTS_EMPTY: "d2h-ins-empty",
            INSERTS_NOSTATUS: "d2h-ins-nostatus",
            DELETES: "d2h-del",
            CONTEXT: "d2h-cntx",
            INFO: "d2h-info"
        };
        ...
        DiffParser.prototype.generateDiffJson = function(diffInput) {
            ...
            startBlock = function(line) {
                saveBlock();
                var values; (values = /^@@ -(\d+),\d+ \+(\d+),\d+ @@.*/.exec(line)) ? currentFile.isCombined = !1 : (values = /^@@@ -(\d+),\d+ -\d+,\d+ \+(\d+),\d+ @@@.*/.exec(line)) ? currentFile.isCombined = !0 : (values = [0, 0], currentFile.isCombined = !1),
                oldLine = values[1],
                newLine = values[2],
                currentBlock = {},
                currentBlock.lines = [],
            ...
            createLine = function(line) {
                ...
                if(tmp.indexOf('M+') != -1 ) {
                    line = line.replace("M+", "+");
                }else if(tmp.indexOf('P+') != -1 ) {
                    line = line.replace("P+", "+");
                }else if(tmp.indexOf('E+') != -1 ) {
                    line = line.replace("E+", "+");
                }else if(tmp.indexOf('N+') != -1 ) {
                    line = line.replace("N+", "+");
                }
    ...
    function() {
        ...
        PrinterUtils.prototype.getHtmlId = function(file) {
            var hashCode = function(text) {
                ...
                for (i = 0, len = text.length; len > i; i++) chr = text.charCodeAt(i),
        ...
        PrinterUtils.prototype.diffHighlight = function(diffLine1, diffLine2, config) {
            var lineStart1, lineStart2, prefixSize = 1;
            ...
            var diff;
            diff = config.charByChar ? jsDiff.diffChars(diffLine1, diffLine2) : jsDiff.diffWordsWithSpace(diffLine1, diffLine2);
            var highlightedLine = "";
            return diff.forEach(function(part) {
                var elemType = part.added ? "ins": part.removed ? "del": null,
                escapedValue = utils.escape(part.value);
                highlightedLine += null !== elemType ? "<" + elemType + ">" + escapedValue + "</" + elemType + ">": escapedValue
            }),
            {
                first: {
                    prefix: lineStart1,
...
function(module, exports) {
    ...
    function buildValues(diff, components, newString, oldString, useLongestToken) {
        for (var componentPos = 0,
        componentLen = components.length,
        newPos = 0,
        oldPos = 0; componentLen > componentPos; componentPos++) {
        ...
        return (lastComponent.added || lastComponent.removed) && diff.equals("", lastComponent.value) && (components[componentLen - 2].value += lastComponent.value, components.pop()),
        components
    }
    ...
    Diff.prototype = {
        diff: function(oldString, newString) {
            ...
            else for (; maxEditLength >= editLength;) {
        ...
        extractCommon: function(basePath, newString, oldString, diagonalPath) {
            for (var newLen = newString.length,
            oldLen = oldString.length,
            newPos = basePath.newPos,
            oldPos = newPos - diagonalPath,
            commonCount = 0; newLen > newPos + 1 && oldLen > oldPos + 1 && this.equals(newString[newPos + 1], oldString[oldPos + 1]);) newPos++,
            oldPos++,
            commonCount++;
...
function(module, exports) {
    ...
    function generateOptions(options, defaults) {
        ...
        else if (options) for (var _name in options) options.hasOwnProperty(_name) && (defaults[_name] = options[_name]);
        return defaults
    }
    exports.__esModule = !0,
...
function(module, exports, __webpack_require__) {
    ...
    function canonicalize(obj, stack, replacementStack) {
        ...
        if ("[object Array]" === objectPrototypeToString.call(obj)) {
            ...
            for (sortedKeys.sort(), i = 0; i < sortedKeys.length; i += 1) key = sortedKeys[i],
...
function(module, exports, __webpack_require__) {
    ...
    function applyPatch(source, uniDiff) {
        ...
        for (var lines = source.split("\n"), hunks = uniDiff.hunks, compareLine = options.compareLine ||
        ...
        errorCount = 0, fuzzFactor = options.fuzzFactor || 0, minLine = 0, offset = 0, removeEOFNL = void 0, addEOFNL = void 0, i = 0; i < hunks.length; i++) {
            for (var hunk = hunks[i], maxLine = lines.length - hunk.oldLines, localOffset = 0, toPos = offset + hunk.oldStart - 1, iterator = _utilDistanceIterator2["default"](toPos, minLine, maxLine); void 0 !== localOffset; localOffset = iterator()) if (hunkFits(hunk, toPos + localOffset)) {
        ...
        for (var i = 0; i < hunks.length; i++) for (var hunk = hunks[i], toPos = hunk.offset + hunk.newStart - 1, j = 0; j < hunk.lines.length; j++) {
            var line = hunk.lines[j],
            operation = line[0],
            content = line.substr(1);
            if (" " === operation) toPos++;
            else if ("-" === operation) lines.splice(toPos, 1);
            else if ("+" === operation) lines.splice(toPos, 0, content),
            toPos++;
            else if ("\\" === operation) {
        ...
        if (removeEOFNL) for (; ! lines[lines.length - 1];) lines.pop();
    ...
    function applyPatches(uniDiff, options) {
        function processIndex() {
            ...
            return index ? void options.loadFile(index,
            function(err, data) {
...
function(module, exports) {
    ...
    function parsePatch(uniDiff) {
        function parseIndex() {
            var index = {};
            for (list.push(index); i < diffstr.length && !/^(Index:|diff -r|@@)/.test(diffstr[i]);) i++;
            var header = /^(?:Index:|diff(?: -r \w+)+) (.*)/.exec(diffstr[i]);
            for (header ? (index.index = header[1], i++, /^===/.test(diffstr[i]) && i++, parseFileHeader(index), parseFileHeader(index)) : (parseFileHeader({}), parseFileHeader({})), index.hunks = []; i < diffstr.length && !/^(Index:|diff -r)/.test(diffstr[i]);) if (/^@@/.test(diffstr[i])) index.hunks.push(parseHunk());
            else {
                if (diffstr[i] && options.strict) throw new Error("Unknown line " + (i + 1) + " " + JSON.stringify(diffstr[i]));
                i++
            }
        }
        ...
        function parseHunk() {
            for (var chunkHeaderIndex = i,
            chunkHeaderLine = diffstr[i++], chunkHeader = chunkHeaderLine.split(/@@ -(\d+)(?:,(\d+))? \+(\d+)(?:,(\d+))? @@/), hunk = {
...
function(module, exports, __webpack_require__) {
        ...
        for (var hunks = [], oldRangeStart = 0, newRangeStart = 0, curRange = [], oldLine = 1, newLine = 1, _loop = function(i) {
            ...
            if (current.lines = lines, current.added || current.removed) {
                ...
                if (!oldRangeStart) {
                    ...
                    prev && (curRange = options.context > 0 ? contextLines(prev.lines.slice( - options.context)) : [], oldRangeStart -= curRange.length, newRangeStart -= curRange.length)
                ...
                if (oldRangeStart) if (lines.length <= 2 * options.context && i < diff.length - 2) {
                    var _curRange2; (_curRange2 = curRange).push.apply(_curRange2, _toConsumableArray(contextLines(lines)))
                } else {
                    var _curRange3, contextSize = Math.min(lines.length, options.context); (_curRange3 = curRange).push.apply(_curRange3, _toConsumableArray(contextLines(lines.slice(0, contextSize))));
                    var hunk = {
    ...
    function createTwoFilesPatch(oldFileName, newFileName, oldStr, newStr, oldHeader, newHeader, options) {
        ...
        ret.push("==================================================================="),
        ret.push("--- " + diff.oldFileName + ("undefined" == typeof diff.oldHeader ? "": "	" + diff.oldHeader)),
        ret.push("+++ " + diff.newFileName + ("undefined" == typeof diff.newHeader ? "": "	" + diff.newHeader));
...
function(module, exports) {
    ...
    function convertChangesToDMP(changes) {
        for (var ret = [], change = void 0, operation = void 0, i = 0; i < changes.length; i++) change = changes[i],
...
function(module, exports, __webpack_require__) { !
    function() {
        ...
        sideBySidePrinter = __webpack_require__(22).SideBySidePrinter;
    ...
    function() {
        ...
        function generateFileHtml(file, config) {
            return file.blocks.map(function(block) {
                for (var lines = '<tr>\n  <td class="d2h-code-linenumber ' + diffParser.LINE_TYPE.INFO + '"></td>\n  <td class="' + diffParser.LINE_TYPE.INFO + '">    <div class="d2h-code-line ' + diffParser.LINE_TYPE.INFO + '">' + utils.escape(block.header) + "</div></td>\n</tr>\n", oldLines = [], newLines = [], processedOldLines = [], processedNewLines = [], i = 0; i < block.lines.length; i++) {
                    var line = block.lines[i],
                    escapedLine = utils.escape(line.content);
                    if (line.type != diffParser.LINE_TYPE.CONTEXT || oldLines.length || newLines.length) if ((line.type != diffParser.LINE_TYPE.INSERTS && line.type != diffParser.LINE_TYPE.INSERTS_MISS && line.type != diffParser.LINE_TYPE.INSERTS_PART&& line.type != diffParser.LINE_TYPE.INSERTS_NOSTATUS&& line.type != diffParser.LINE_TYPE.INSERTS_EMPTY) || oldLines.length || newLines.length)  if (line.type != diffParser.LINE_TYPE.DELETES || newLines.length) if ((line.type == diffParser.LINE_TYPE.INSERTS || line.type == diffParser.LINE_TYPE.INSERTS_MISS || line.type == diffParser.LINE_TYPE.INSERTS_PART || line.type == diffParser.LINE_TYPE.INSERTS_NOSTATUS || line.type == diffParser.LINE_TYPE.INSERTS_EMPTY) && oldLines.length > newLines.length)   newLines.push(line);
                    else {
                        var oldLine, newLine, j = 0;
                        ...
                        } else lines += processLines(oldLines, newLines);
                        oldLines = [],
                        newLines = [],
                        processedOldLines = [],
                        processedNewLines = [],
                        i--
                    } else oldLines.push(line);
                    else lines += generateLineHtml(line.type, line.oldNumber, line.newNumber, escapedLine);
                    else lines += generateLineHtml(line.type, line.oldNumber, line.newNumber, escapedLine)
                }
                return lines += processLines(oldLines, newLines)
            }).join("\n")
        }
        ...
        function generateLineHtml(type, oldNumber, newNumber, content, prefix) {
            ...
            var htmlContent = "";
            return content && (htmlContent = '<span class="d2h-code-line-ctn">' + content + "</span>"),
        ...
        LineByLinePrinter.prototype.generateLineByLineJsonHtml = function(diffFiles, config) {
            return '<div class="d2h-wrapper">\n' + diffFiles.map(function(file) {
                var diffs;
                return diffs = file.blocks.length ? generateFileHtml(file, config) : generateEmptyDiff(),
    ...
    function() {
        ...
        function generateSideBySideFileHtml(file, config) {
            ...
            file.blocks.forEach(function(block) {
                ...
                for (var oldLines = [], newLines = [], tmpHtml = "", i = 0; i < block.lines.length; i++) {
                    var line = block.lines[i],
                    escapedLine = utils.escape(line.content);
                    if (line.type != diffParser.LINE_TYPE.CONTEXT || oldLines.length || newLines.length) if ((line.type != diffParser.LINE_TYPE.INSERTS &&line.type != diffParser.LINE_TYPE.INSERTS_MISS &&line.type != diffParser.LINE_TYPE.INSERTS_PART &&line.type != diffParser.LINE_TYPE.INSERTS_EMPTY &&line.type != diffParser.LINE_TYPE.INSERTS_NOSTATUS) || oldLines.length || newLines.length) if (line.type != diffParser.LINE_TYPE.DELETES || newLines.length) if ((line.type == diffParser.LINE_TYPE.INSERTS ||line.type == diffParser.LINE_TYPE.INSERTS_MISS ||line.type == diffParser.LINE_TYPE.INSERTS_PART ||line.type == diffParser.LINE_TYPE.INSERTS_EMPTY ||line.type == diffParser.LINE_TYPE.INSERTS_NOSTATUS) && oldLines.length > newLines.length)   newLines.push(line);
                    else {
                        var oldLine, newLine, j = 0;
                        if (oldLines.length === newLines.length) for (j = 0; j < oldLines.length; j++) {
                            oldLine = oldLines[j],
                            newLine = newLines[j],
                            config.isCombined = file.isCombined;
                            var diff = printerUtils.diffHighlight(oldLine.content, newLine.content, config);
                        ...
                        } else tmpHtml = processLines(oldLines, newLines),
                    ...
                    else fileHtml.left += generateSingleLineHtml(diffParser.LINE_TYPE.CONTEXT, "", "", ""),
                    fileHtml.right += generateSingleLineHtml(line.type, line.newNumber, escapedLine);
                    else fileHtml.left += generateSingleLineHtml(line.type, line.oldNumber, escapedLine),
                    fileHtml.right += generateSingleLineHtml(line.type, line.newNumber, escapedLine)
                }
        ...
        SideBySidePrinter.prototype.generateSideBySideJsonHtml = function(diffFiles, config) {
            return '<div class="d2h-wrapper">\n' + diffFiles.map(function(file) {
                var diffs;
                return diffs = file.blocks.length ? generateSideBySideFileHtml(file, config) : generateEmptyDiff(),
