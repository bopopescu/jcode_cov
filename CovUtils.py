...
def run_cmd(cmd, exception_on_errors=True):
    ...
    if return_code != 0:
        ...
        clog.info("{}; stdout: {}; stderr: {}".format(err_msg, stdout, stderr))
        if exception_on_errors:
...
def handle_error(func, path, exc_info):
    ...
    :param exc_info:
    """
    clog.info("Handling Error for file ", path)
    clog.info(exc_info)
    if not os.access(path, os.W_OK):
        clog.info("Re-handle {}".format(path))
...
def extract_pack(pack, target_dir):
    ...
    try:
        ...
        clog.error("BadZipFile: {}".format(zb))
    except zipfile.LargeZipFile as zl:
        clog.error("LargeZipFile: {}".format(zl))
    except Exception:
